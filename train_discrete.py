...
def get_args():
    ...
    parser.add_argument('--load_path', type=str, default='trained_models/Net_discrete/checkpoint_114.pth.tar',
    ...
    parser.add_argument('--experiment_name', type=str, default='discrete', help='exp name')
...
def main():
    ...
    sintel_label_dir = '/media/yotamg/bd0eccc9-4cd5-414c-b764-c5a7890f9785/Yotam/Sintel/left_filtered/GT'
    ...
    if just_sintel:
        ...
        tau_label_dir = '/media/yotamg/bd0eccc9-4cd5-414c-b764-c5a7890f9785/Yotam/Tau-agent/left_filtered/GT'
        ...
        tau_train_label_filelist = [img.replace(tau_train_dir, tau_label_dir).replace('_1100_maskImg.png', '_GT.dpt')
                                    for img in tau_train_filelist]
        tau_test_label_filelist = [img.replace(tau_test_dir, tau_label_dir).replace('_1100_maskImg.png', '_GT.dpt') for
                                   img in tau_test_filelist]
    ...
    train_dataset = Dataset(image_filelist=train_filelist, label_filelist=train_label_filelist, train=True, pickle_name='train_discrete_segmentation.pickle', transforms=transform, target_mode=args.target_mode)
    test_dataset  = Dataset(image_filelist=test_filelist,  label_filelist=test_label_filelist,  train=False, pickle_name='test_discrete_segmentation.pickle', transforms=transform, target_mode=args.target_mode)
